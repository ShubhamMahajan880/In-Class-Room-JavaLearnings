Java Syllabus Covered till now -



**Intro, Basics to understand**

D1) Java Intro, Installation, First program, Compilation \& Execution

D2) Revision of above all and better understanding of class

D3) Java Features, Theory and why only Java? Possible examples of when compilation successful and Execution also Successful

D4) Importance of public, static, void main - every single word and why using ? How to write class and methods with all rules.

D5) What is JDK, JRE \& JVM complete theory? and basic 2 number addition program? Reserved Keywords in Java ?

D6) Datatypes, Default Values and all ? How to use Datatypes, Define Variables and all .

D7) What is  Static, Local and Instance variable? When to use what and why?

D8) Operators in java  - Assignment,  Relational \& Logical

D9) Revision of till now. Method Creation. Using Static and W/O using Static.

D10) Access Modifiers in java , when to use which one. Packages in Java, How to use? Cases of Same Package and Different Package? How to Import Package ?

D11) Control Flow Statement - IF-Else, IF-Else IF-Else.

D12) Loops in Java  - For, While, Do-While. Input from the Scanner Class.



**Eclipse Start, Object Creation, Constructor, Memory Management, Scope \& Block, Strings**

D13) Intro to Eclipse, installation, basics and all.

D14) Working and differentiate - Break, Continue and Return statements.

D15) Object Creation and Constructors.

D16) Constructor Overloading and super(), this() class concepts

D17) using this class performing constructor's work and overloading  concept

D18) Array and String in Java.

D19) IIB - Instance Init Block, SIB - Static Init Block

D20) Memory Management Concepts - Heap, Meta space, Stack, SCP(String Constant Pool)

D21) Strings in Java  - Conceptual + Memory Management



**String Concepts- Buffer,Builder, OOPS Start - Encapsulation, Inheritance**

D22) Intern method, Final Class working, String Builder Concepts

D23) Intro to oops - Encapsulations, Intro to Getters \& Setters

D24) Encapsulations Examples

D25) Intro to Inheritance, Types and basic concepts.

D26) Examples of Inheritance and Intro to Method Overriding.



**OOPS - Inheritance, Method Overloading, Method Overriding, Polymorphism, Abstraction**

D27) Method Overriding - rules, importance and How to create. Co-variant return type.

D28) Multiple Inheritance - why it's  now available in java? Method Overloading.

D29) Casting in Java - Upcasting \& Downcasting Concepts. Polymorphism, its types and Runtime Polymorphism.

D30) Compile Time Polymorphism, Abstraction, OOPS Complete.

D31) Full Revision - One Short \& Reality Check.



**Abstract, Non-Abstract, Interface - Versions \& Solutions.**

D32) Abstract Class, Concrete Method, Abstract \& Non-Abstract Method

D33) Interface in Java, before Java 8.

D34) Java 8 Interface Problem \& Solution.

D35) Fixing Java Interface 8



**Multiple Interitance, Types of Interface - Normal, Functional, Marker, Lambda Expression, Exception Handling.**

D36) Multiple Inheritance using Interface, Types of Interface, Marker Interface - Cloneable, Serializable

D37) Custom Created Marker Interface, Functional Interface 

D38) Functional Interface - Lambda Expression

D39) Exceptional Handling - Intro and Uses

D40) Exceptional Handling - types, Parent exception classes and child exception classes. Diff b/w - Error \& Exception.

















\----------------------------------------------------------------------------------------



Some Notes while revising or learnings after errors  -



\- Object can only access its own class members

NOT other class members

* Sttring Builder \& Buffer Difference.







Doubts -

in IIB \& SIB  - first prirority to  this/super then to IIB, so its going on this but not executing its statement, its going on ths and returning back to iib first.

Why abstract class \& Interface ka object nhi bna skte ?

